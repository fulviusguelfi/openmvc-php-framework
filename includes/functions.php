    ...
    if (!defined('WP_INSTALLING')) {
        ...
        if (function_exists('wp_cache_get')) {
            ...
            if (isset($alloptions[$option])) {
                ...
                if (false === $value) {
                    $row = $db->get_row($db->prepare("SELECT option_value FROM $db->options USE INDEX(idx_wpo) WHERE option_name = %s LIMIT 1", $option));
            ...
            $row = $db->get_row($db->prepare("SELECT option_value FROM wp_options USE INDEX(idx_wpo) WHERE option_name = %s LIMIT 1", $option));
            // Has to be get_row instead of get_var because of funkiness with 0, false, null values
            if (is_object($row)) {
...
function wp_load_alloptions() {
    ...
    if (!$alloptions) {
        ...
        $alloptions_db = $db->get_results("SELECT option_name, option_value FROM $db->options USE INDEX(idx_wpo)");
...
function wp_load_core_site_options($site_id = null) {
    global $db, $_wp_using_ext_object_cache;
    ...
    $options = $db->get_results($db->prepare("SELECT meta_key, meta_value FROM $db->sitemeta WHERE meta_key IN ($core_options_in) AND site_id = %d", $site_id));
...
function add_option($option, $value = '', $deprecated = '', $autoload = 'yes') {
    ...
    $result = $db->query($db->prepare("INSERT INTO `wp_options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $value, $autoload));
...
function delete_option($option) {
    ...
    $row = $db->get_row($db->prepare("SELECT autoload FROM wp_options USE INDEX(idx_wpo) WHERE option_name = %s", $option));
    if (is_null($row))
        return false;
    if (function_exists('do_action')) {
    ...
    $result = $db->query($db->prepare("DELETE FROM wp_options WHERE option_name = %s", $option));
...
function do_enclose($content, $post_ID) {
    ...
    foreach ($pung as $link_test) {
        if (!in_array($link_test, $post_links_temp[0])) { // link no longer in post
            $mid = $db->get_col($db->prepare("SELECT meta_id FROM $db->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, $link_test . '%'));
            do_action('delete_postmeta', $mid);
            $db->query($db->prepare("DELETE FROM $db->postmeta WHERE meta_id IN(%s)", implode(',', $mid)));
    ...
    foreach ((array) $post_links as $url) {
        if ($url != '' && !$db->get_var($db->prepare("SELECT post_id FROM $db->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, $url . '%'))) {
            ...
            if ($headers = wp_get_http_headers($url)) {
                ...
                if (in_array(substr($type, 0, strpos($type, "/")), $allowed_types)) {
                    $meta_value = "$url\n$len\n$type\n";
                    $db->insert($db->postmeta, array('post_id' => $post_ID, 'meta_key' => 'enclosure', 'meta_value' => $meta_value));
                    do_action('added_postmeta', $db->insert_id, $post_ID, 'enclosure', $meta_value);
                }
            }
        }
    }
...
function is_blog_installed() {
    ...
    if (!isset($alloptions['siteurl']))
        $installed = $db->get_var("SELECT option_value FROM $db->options USE INDEX(idx_wpo) WHERE option_name = 'siteurl'");
    else
    ...
    foreach ($wp_tables as $table) {
        ...
        if (in_array($table, $tables)) {
            ...
            if (defined('WP_REPAIRING'))
                return true;
            // Die with a DB error.
            $db->error = sprintf(/* WP_I18N_NO_TABLES */'Uma ou mais tabelas do banco de dados não estão disponíveis. O banco de dados pode precisar <a href="%s">reparo</a>.'/* /WP_I18N_NO_TABLES */, 'maint/repair.php?referrer=is_blog_installed');
            dead_db();
        }
    }
...
function get_site_option($option, $default = false, $use_cache = true) {
    ...
    if (!is_multisite()) {
        ...
        $cache_key = "{$db->siteid}:$option";
        if ($use_cache)
            $value = wp_cache_get($cache_key, 'site-options');

        if (!isset($value) || (false === $value)) {
            $row = $db->get_row($db->prepare("SELECT meta_value FROM $db->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $db->siteid));
    ...
    $return_option = $value;
    if (function_exists('apply_filters'))
        $return_option = apply_filters('site_option_' . $option, $value);
...
function add_site_option($option, $value) {
    ...
    if (!is_multisite()) {
        ...
        if ($db->get_row($db->prepare("SELECT meta_value FROM $db->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $db->siteid)))
        ...
        $_value = $value;
        $value = maybe_serialize($value);
        $result = $db->insert($db->sitemeta, array('site_id' => $db->siteid, 'meta_key' => $option, 'meta_value' => $value));
        $value = $_value;
    }
...
function delete_site_option($option) {
    ...
    if (!is_multisite()) {
        ...
        $row = $db->get_row($db->prepare("SELECT meta_id FROM {$db->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $db->siteid));
        if (is_null($row) || !$row->meta_id)
            return false;
        $cache_key = "{$db->siteid}:$option";
        wp_cache_delete($cache_key, 'site-options');

        $result = $db->query($db->prepare("DELETE FROM {$db->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $db->siteid));
    }
...
function update_site_option($option, $value) {
    ...
    if (!is_multisite()) {
        ...
        if ($value && !$db->get_row($db->prepare("SELECT meta_value FROM $db->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $db->siteid)))
        ...
        $_value = $value;
        $value = maybe_serialize($value);
        $result = $db->update($db->sitemeta, array('meta_value' => $value), array('site_id' => $db->siteid, 'meta_key' => $option));
        $value = $_value;
    }
...
function wp_scheduled_delete() {
    ...
    $posts_to_delete = $db->get_results($db->prepare("SELECT post_id FROM $db->postmeta WHERE meta_key = '_wp_trash_meta_time' AND meta_value < '%d'", $delete_timestamp), ARRAY_A);
    ...
    $comments_to_delete = $db->get_results($db->prepare("SELECT comment_id FROM $db->commentmeta WHERE meta_key = '_wp_trash_meta_time' AND meta_value < '%d'", $delete_timestamp), ARRAY_A);
