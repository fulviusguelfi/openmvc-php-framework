...
//    
    ...
    public function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
        echo_error("Essa classe não está finalizada. Contribua com OpenMvc no arquivo app/includes/drivers/pdo.sql", "PDO-DIE", true);
        $this->connect($dbuser, $dbpassword, $dbname, $dbhost);
    }
     ...
     */
    private function connect($dbUser, $dbPassword, $dbName, $dbHost) {
        set_time_limit(30);
        global $wpdb;
    ...
    public function get_results($query = null, $output = OBJECT) {
        ...
        if ($query) {
            $this->query($query);
            $this->last_result = $this->getQueryResults();
        } else {
            return null;
        }
    ...
    private function executeQuery($statement) {
        ...
        if ($result === false) {
            ...
            $this->last_result = $this->_results;
        }
...
HTML;
    ...
    private function setError($line, $function, $message) {
        $X = array("line" => $line, "function" => $function);
        $this->errors[] = $X;
        $this->last_error = $X;
